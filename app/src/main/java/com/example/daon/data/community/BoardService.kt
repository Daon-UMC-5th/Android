...
interface BoardService {
    ...
    fun getPost(@Path("boardId") boardId: Int): Call<PostOneListResponseDto>
    // 게시판 작성
    ...
    fun updatePost(@Path("boardId") boardId: Int, @Body post: PostWriteRequestDto): Call<PostWriteResponseDto>
    // 게시판 삭제
    @DELETE("/board/write/delete/{boardId}")
    fun deletePost(@Path("boardId") boardId: Int): Call<PostWriteResponseDto>
    // 좋아요 추가
    @POST("/board/like-up/{boardId}")
    fun likeUp(@Path("boardId") boardId: Int): Call<LikeResponse>
    // 좋아요 삭제
    @DELETE("/board/like-down/{boardId}")
    fun likeDown(@Path("boardId") boardId: Int): Call<LikeResponse>
    // 특정 글 좋아요 수 가져오기
    @GET("/board/like/{boardId}")
    fun getLikesForPost(@Path("boardId") boardId: Int): Call<LikeResponse>
    // 좋아요 전체 가져오기
    @GET("/board/all-like")
    fun getAllLikes(@Query("offset") offset: Int): Call<LikeResponse>
    // 스크랩 추가
    @POST("/board/add-scrape/{boardId}")
    fun addScrape(@Path("boardId") boardId: Int): Call<ScrapeResponse>
    // 스크랩 삭제
    ...
    fun likeUpComment(@Path("commentId") commentId: Int): Call<LikeResponse>
    // 댓글 좋아요 삭제
    @DELETE("/board/comment/like-down/{commentId}")
    fun likeDownComment(@Path("commentId") commentId: Int): Call<LikeResponse>
    @Multipart
    @POST("/upload/board")
